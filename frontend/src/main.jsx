...
/* ==================  CONFIG: replace the split line with your exact coords  ==================
   Define the line that splits Sheen as two points (lon,lat). We’ll classify addresses against it.
   TODO: Replace both points with the exact “split” you give me next.
*/
const SHEEN_SPLIT = {
  A: { lon: -0.2705, lat: 51.4660 }, // example near Mortlake/Sheen border
  B: { lon: -0.2640, lat: 51.4545 }, // example further south
};
/* ============================================================================================= */
...
/* classify left/right of the A->B line (lon=x, lat=y) */
function sideOfLine(A, B, P) {
  const val = (B.lon - A.lon) * (P.lat - A.lat) - (B.lat - A.lat) * (P.lon - A.lon);
  return val > 0 ? "left" : "right";
}

/* free geocoder (no keys): Nominatim — light usage only */
...
function Details({ onNext, state, setState }) {
  ...
  return (
    <div className="panel">
      ...
      <input placeholder="Address (full address — we’ll route left/right automatically on confirm)" value={v.address} onChange={(e)=>setV({...v, address:e.target.value})}/>
...
function Calendar({ onNext, onBack, state, setState }) {
  ...
  useEffect(() => {
    ...
    fetch(API + "/availability", {
      ...
      body: JSON.stringify({
        ...
        addons: state.addons || [],
        area: "right" // area is decided on Confirm; backend buffer doesn’t use it yet
      }),
    })
      .then(r=>r.json())
  ...
  const grouped = useMemo(()=>{
    const g={};
    for(const s of slots){ const k=dkey(s.start_iso); (g[k] ||= []).push(s); }
    return g;
  },[slots]);
  ...
  return (
    <div className={cx("panel", loading && "loading")}>
      ...
      {days.length === 0 && !loading && <div className="muted">No slots available.</div>}
      {days.map(k=>(
...
function Confirm({ onBack, state, setState }) {
  ...
  async function confirm() {
    // 1) convert address -> lon/lat; 2) classify side via your split; 3) send booking
    let area = "right";
    try {
      const p = await geocodeAddress(state.customer.address);
      area = sideOfLine(SHEEN_SPLIT.A, SHEEN_SPLIT.B, p);
    } catch (e) {
      // if geocode fails, default 'right'
      area = "right";
    }
    ...
    if (res.ok) {
      alert(`Booking confirmed (${area.toUpperCase()}). Check your email for confirmation.`);
      setState({});
      location.href = "/";
    } else {
