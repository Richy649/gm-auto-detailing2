...
const addMonthsYYYYMM = (yyyyMM,delta)=>{ const [y,m]=yyyyMM.split("-").map(Number); const d=new Date(Date.UTC(y,m-1,1,12)); d.setUTCMonth(d.getUTCMonth()+delta); return `${d.getUTCFullYear()}-${pad(d.getUTCMonth()+1)}`; };

/* iFrame helpers (Squarespace) */
function reportHeight(){ const h = Math.max(document.body.scrollHeight, document.documentElement.scrollHeight, window.innerHeight); try{ window.parent.postMessage({type:"GM_HEIGHT", height:h},"*"); }catch{} }
...
/* Buttons / Cards */
const Button = ({children,className,...p}) => <button className={cx("gm btn",className)} {...p}>{children}</button>;
const PrimaryButton = (p) => <Button className="primary" {...p} />;
    ...
    <div className={cx("gm card", selected && "selected")} onClick={onClick} role="button">
      ...
      {strike ? (
        <div style={{display:"flex",alignItems:"baseline",gap:8}}>
          <div className="gm muted" style={{textDecoration:"line-through"}}>{fmtGBP(strike)}</div>
...
// UK-ish: allow 0XXXXXXXXXX or +44XXXXXXXXXX (10 digits after country code). We strip spaces/dashes.
...
function Details({ state, setState, onNext }) {
  ...
  function go() {
    ...
    if (!hasTap)                         return alert("Please confirm you have an outdoor tap to continue.");
...
function Services({ state, setState, onBack, onNext, cfg }) {
  ...
  const discount = Number(cfg.discount_gbp || 0);
  ...
  function go(){
    ...
    setState((s)=> ({ ...s, service_key: svc, addons, selectedDayKey: s.selectedDayKey || null, selectedSlot: s.selectedSlot || null, membershipSlots: s.membershipSlots || [] }));
    setTimeout(reportHeight,60);
    onNext();
  }

  const price = (k)=> (sCfg[k]?.price ?? 0);
  const eff = (k)=> Math.max(0, price(k) - discount);
  ...
  return (
    <div className="gm page-section gm-booking wrap">
      <div className="gm panel wider">
        <div className="gm h2 center">£{discount} OFF – first 30 bookings 🎉</div>

        <div className="gm cards">
          <ServiceCard title={sCfg.exterior?.name || "Exterior Detail"} price={eff("exterior")} strike={discount?price("exterior"):0} selected={svc==="exterior"} onClick={()=>setSvc("exterior")} />
          <ServiceCard title={sCfg.full?.name || "Full Detail"} price={eff("full")} strike={discount?price("full"):0} selected={svc==="full"} onClick={()=>setSvc("full")} />
          <ServiceCard title={sCfg.standard_membership?.name || "Standard Membership (2 Exterior)"} price={eff("standard_membership")} strike={discount?price("standard_membership"):0} selected={svc==="standard_membership"} onClick={()=>setSvc("standard_membership")} />
          <ServiceCard title={sCfg.premium_membership?.name || "Premium Membership (2 Full)"} price={eff("premium_membership")} strike={discount?price("premium_membership"):0} selected={svc==="premium_membership"} onClick={()=>setSvc("premium_membership")} />
        </div>

        <div className="gm h2 center" style={{ marginTop: 8 }}>Add-ons (optional)</div>
        <div className="gm addon-benefits two-col">
          <AddonCard title={aCfg.wax?.name || "Full Body Wax"} price={aCfg.wax?.price ?? 10}
          ...
          <AddonCard title={aCfg.polish?.name || "Hand Polish"} price={aCfg.polish?.price ?? 22.5}
...
function Calendar({ state, setState, onBack, onGoTimes }) {
  ...
  React.useEffect(()=>{
    if (!state.availability || state.availability.month !== monthKey) loadMonth(monthKey);
      ...
      <div className="gm panel wider">
        <div className="gm monthbar-grid">
          <div className="gm monthnav-left">
            <Button className="gm btn nav" disabled={!canPrev} onClick={()=> canPrev && loadMonth(addMonthsYYYYMM(monthKey,-1))}>Previous</Button>
          </div>
          ...
          <div className="gm monthnav-right">
            ...
            <Button className="gm btn nav" disabled={!canNext} onClick={()=> canNext && loadMonth(addMonthsYYYYMM(monthKey,+1))}>Next</Button>
          </div>
        </div>
      ...
      <div className="gm panel wider">
        ...
        {isMembership && (
          <div className="gm actions space" style={{ marginBottom: 8 }}>
            ...
            <Button onClick={()=> onBack()}>See other days</Button> {/* extra button for membership */}
          </div>
        )}
...
function Confirm({ state, setState, onBack }) {
  ...
  async function pay(){
    ...
    if (!d?.ok || !d?.url) return alert(d?.error || "Payment failed to initialize.");
        ...
        <div className="gm twocol">
          <div>
            <div className="gm card" style={{marginBottom:10}}>
              ...
              <div>Outdoor tap: {state.has_tap ? "Yes" : "No"}</div>
            </div>
          ...
          <div className="gm card">
            ...
            {discount ? (
              <div style={{display:"flex",gap:8,alignItems:"baseline"}}>
                <div className="gm total" style={{textDecoration:"line-through", fontSize:24}}>{fmtGBP(preDiscount)}</div>
...
function App(){
  const [state, setState] = React.useState({
    ...
    availability:null, monthKey: toDateKey(new Date()).slice(0,7), config:null,
  });
  ...
  React.useEffect(()=>{
    ...
    fetch(`${API}/availability?service_key=${encodeURIComponent(state.service_key)}&month=${state.monthKey}`)
      .then(r=>r.json()).then(d=>{
        ...
        setState(s=> ({...s, availability:d, selectedDayKey: s.selectedDayKey || keys[0] || null }));
      })
      .finally(()=> setTimeout(reportHeight,60));
