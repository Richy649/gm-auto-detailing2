...
/* ============ In-memory fallback ============ */
const mem = {
  bookings: [], // [{start_iso,end_iso,service_key,session_id,...}]
  holds: []     // [{hold_key,session_id,start_iso,end_iso,expires_at,...}]
};
...
export async function addHold({ hold_key, service_key, start_iso, end_iso, customer, session_id = null }) {
  ...
  if (pool) {
    ...
    // Check conflicts vs existing holds (not expired) and bookings
    const now = nowUtc();
    ...
    mem.holds.push({ hold_key, session_id, service_key, customer_json: customer || {}, start_iso, end_iso, expires_at });
    return { ok: true };
  }
}
...
export async function isSlotFree(start_iso, end_iso) {
  ...
  if (pool) {
    ...
    const { rows: rh } = await pool.query(
    ...
    const hasHold = mem.holds.some(h => h.start_iso === start_iso && h.end_iso === end_iso && DateTime.fromISO(h.expires_at) > now);
    return !hasHold;
  }
}
...
export async function promoteHoldsToBookingsBySession(session_id, metadata) {
  // Move all holds for this session → bookings; delete holds.
  if (pool) {
    const client = await pool.connect();
    try {
      await client.query("begin");
      const { rows: holds } = await client.query(
        "select * from holds where session_id=$1",
        [session_id]
      );
      for (const h of holds) {
        // Insert booking (ignore duplicate)
        await client.query(
